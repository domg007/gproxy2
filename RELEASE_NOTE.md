...
- provider 作用域下的 OpenAI 兼容 `v1/models` 与 `v1/chat/completions` 路由新增可选 `credential_id` 定向参数，便于后台直接指定某个凭证执行校验请求。

#### 修复

- 批量测试凭证时，不再把瞬时上游故障直接标记为 dead；`429/500/502/503/504` 现在会记录为 `partial`。
- Codex 遇到 `403` HTML 错误页（包括常见 Cloudflare 拦截/中转页）时，不再直接把凭证判定为 dead，而是按瞬时故障处理。

#### 兼容性

- 不需要额外的数据迁移。
- 现有 dead / partial 凭证状态语义保持不变；本次仅让后台校验流程和 Codex 边缘故障判断更温和，减少误杀。
