...
`gproxy-protocol` 是 [gproxy](https://github.com/LeenHawk/gproxy) 项目的协议类型库，覆盖
**Anthropic Claude**、**OpenAI**（ChatCompletions + Responses）和 **Google Gemini** 三套主流
LLM API 的请求、响应、流式事件与共享类型，并通过 `transform` 模块提供跨协议转换。

零 HTTP 依赖，纯 `serde` 数据 + 类型化转换器。

## 快速开始

通过 `TryFrom` 进行类型化的跨协议转换：
...
当源 / 目标协议只能在运行时（例如来自配置）才能确定时，使用基于枚举键的运行时分发：
