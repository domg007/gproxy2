...
CMD ["/bin/sh", "-c", "set -eu; host=\"${GPROXY_HOST:-0.0.0.0}\"; port=\"${GPROXY_PORT:-8787}\"; data_dir=\"${GPROXY_DATA_DIR:-/app/data}\"; admin_key=\"${GPROXY_ADMIN_KEY:-pwd}\"; dsn=\"${GPROXY_DSN:-sqlite:///app/data/db/gproxy.db?mode=rwc}\"; proxy=\"${GPROXY_PROXY:-}\"; case \"$host\" in '${GPROXY_HOST}'|'') host='0.0.0.0' ;; esac; case \"$port\" in '${GPROXY_PORT}'|'') port='8787' ;; esac; case \"$data_dir\" in '${GPROXY_DATA_DIR}'|'') data_dir='/app/data' ;; esac; case \"$admin_key\" in '${GPROXY_ADMIN_KEY}'|'') admin_key='pwd' ;; esac; case \"$dsn\" in '${GPROXY_DSN}') dsn='' ;; esac; case \"$proxy\" in '${GPROXY_PROXY}') proxy='' ;; esac; set -- /usr/local/bin/gproxy --host \"$host\" --port \"$port\" --admin-key \"$admin_key\" --data-dir \"$data_dir\"; [ -n \"$dsn\" ] && set -- \"$@\" --dsn \"$dsn\"; [ -n \"$proxy\" ] && set -- \"$@\" --proxy \"$proxy\"; exec \"$@\""]
