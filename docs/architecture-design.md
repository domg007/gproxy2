...
**每凭证不同出站代理**:wreq 的 proxy 绑在 client 实例上、无法逐请求切换,所以
native 的 `WreqClient` 内部维护一个**按 `(proxy_url, 是否 TLS 伪装)` 缓存的 client 池**,
发请求时按**选中凭证**解析出的 proxy 选用对应 client(懒构建 + 复用)。代理端点
**provider 设默认、credential 覆盖**(见 §8-B `providers.settings_json` / `credentials.proxy_url`);
全局兜底仍可用 `--upstream-proxy-url` / `GPROXY_UPSTREAM_PROXY_URL`。
**edge 不支持本地 proxy**,走平台 `fetch`,per-credential proxy 在 edge 忽略。
