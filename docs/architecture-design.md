...
- **各渠道已知冷却信号(2026-06-11 记,随 M7b 调研)**:目前只记录信号,逐渠道冷却钩子仍是
  ...
  - **copilot_cli / github**:GitHub 速率头(`x-ratelimit-remaining` / `x-ratelimit-reset`)给出
    窗口剩余与重置时刻。
...
天然安全,无快照结构变更。memory 单实例 / edge 的 `subscribe` 为 no-op(无需跨实例失效)。
...
**M7a**:client 池已按 (proxy, fingerprint) 键;header 层 emulation 生效,TLS/HTTP2 指纹预设随各渠道(M7b)。wreq 6.0.0-rc.29。
...
**其余 5 个(vertex / copilot_cli / geminicli / antigravity / kiro)是 header-only**:只注入鉴权 + UA/客户端指纹头,
...
### 19.2 信任锚(供应链安全,最关键)
- **签名清单**:发布一份**版本清单**(manifest JSON)+ 每平台制品。清单字段:
  `{ version, notes_url, min_compatible_data_version, artifacts: [{ target_triple, url, sha256, size }], signature }`。
- **签名校验不可绕过**:清单用 **ed25519**(minisign 风格)签名;**公钥编译进二进制**(build 期 const)。
  自更新流程:下载制品 → sha256 比对清单 → 清单签名用内嵌公钥验签 → 任一不符即**拒绝**。
  **无有效签名绝不替换二进制**。这是自替换二进制的硬底线。
- 传输走 HTTPS(TLS)+ 签名,纵深防御。`ed25519-dalek` 已在依赖树(jsonwebtoken rust_crypto 带入),
  或用 `minisign-verify`,不引重依赖。

### 19.3 版本检查与策略
- 当前版本 = `CARGO_PKG_VERSION`(`/version` 已暴露)。检查 = 经 `UpstreamClient`(proxy-aware)拉清单,
  semver 比较。
- 触发:① admin API(`GET /admin/update/check`);② 可选低频后台轮询(默认关或按天)。**轮询绝不自动应用**。
- 策略落 `instance_settings.update_channel`(该字段 §8-B 已预留):`off | notify | manual | auto`,
  默认 **`manual`**(检查+上报,管理员批准才应用)。`auto` = 按计划检查+应用+重启(opt-in,有风险);
  `notify` = 仅上报可用性。
...
### 19.9 admin API 面(实现时随 M10 风格落地)
