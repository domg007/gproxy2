...
上游 HTTP 发送抽象成 **`UpstreamClient` trait**,**与具体 HTTP 客户端无关**——这是让边缘可达、也让客户端可换的关键接缝。**wreq 不是"唯一的 native 客户端",只是其中一个实现**(提供 TLS 指纹伪装能力)。可并存多个实现:
- **wreq 实现**:提供 `tls_emulation`,给需要伪装的渠道(目前仅 `chatgpt`)。
- **普通客户端实现**(reqwest / hyper 等):不需要伪装的渠道用,更轻。
- **edge 实现(后续)**:平台 `fetch`(无 TLS 控制)。

渠道按能力(`requires_tls_emulation` 等)挑/要求合适的客户端,架构按能力路由,而非把整个 native 传输绑死在 wreq 上。
