...
文件路径、不带任何 secret。**仅 FILE backend 完整往返**(其 `Some(id)` upsert 在未命中时按显式 id 插入);
**DB backend** 的 upsert 仍在显式 id 未命中时报错(M2 gap)→ 用 import 给空 `--persistence=db` 库做种尚未支持,
留待 M10 或专门的 db-upsert 任务移植 insert-with-id 语义。
