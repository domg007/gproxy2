...
pub(super) async fn fetch_cnb_release_tag(
    ...
    let manifest_url = cnb_manifest_url_for_channel(update_channel);
    let response = client
        .get(&manifest_url)
        ...
        .send()
        .await
        .map_err(|err| format!("fetch_cnb_manifest:{manifest_url}:{err}"))?;
    ...
    if !response.status().is_success() {
        ...
        return Err(format!("fetch_cnb_manifest_status_{status}: {body}"));
    }
...
pub(super) async fn fetch_cnb_release_asset(
    ...
    if let Some(channel) = manifest.channel.as_deref() {
        ...
        if normalized != update_channel {
            return Err(format!(
                "cnb_manifest_channel_mismatch: expected={update_channel} actual={normalized}"
            ));
        }
    }
    ...
    Ok((manifest_tag, {
        let download_url = resolve_cnb_asset_url(&manifest_url, asset.url.as_str());
        let expected_sha256 = if let Some(raw) = asset.sha256.as_deref() {
            Some(normalize_sha256_hex(raw).ok_or_else(|| {
                format!("cnb_manifest_invalid_sha256:asset={target_asset}:value={raw}")
            })?)
        } else {
            None
        };
        let sha256_url = asset
            .sha256_url
            .as_deref()
            .map(|value| resolve_cnb_asset_url(&manifest_url, value))
...
fn cnb_manifest_url_for_channel(update_channel: &str) -> String {
    format!(
...
fn resolve_cnb_asset_url(manifest_url: &str, raw_url: &str) -> String {
