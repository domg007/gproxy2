...
pub(super) fn build_self_update_client(proxy: Option<String>) -> Result<wreq::Client, String> {
    let proxy = proxy
...
pub(super) async fn download_bytes_with_redirects(
    client: &wreq::Client,
    url: &str,
    max_redirects: usize,
...
pub(super) fn target_release_asset_name() -> Result<String, String> {
...
pub(super) fn extract_binary_from_zip(zip_bytes: &[u8]) -> Result<Vec<u8>, String> {
...
#[cfg(not(windows))]
pub(super) fn install_binary_bytes(binary: Vec<u8>) -> Result<(), String> {
...
#[cfg(windows)]
pub(super) fn stage_windows_binary_bytes(binary: Vec<u8>) -> Result<PathBuf, String> {
    let current = env::current_exe().map_err(|err| format!("current_exe:{err}"))?;
    let parent = current
        .parent()
        .ok_or_else(|| "current_exe_parent_missing".to_string())?;
...
fn temp_update_path(parent: &Path) -> PathBuf {
...
pub(super) fn schedule_self_restart(staged_binary_path: Option<PathBuf>) -> Result<(), String> {
...
pub(super) fn maybe_fix_deleted_exe_path(exe: PathBuf) -> PathBuf {
    if exe.exists() {
        return exe;
    }
