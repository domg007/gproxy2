...
impl OpenAiResponseToGeminiStream {
    ...
    fn map_output_item(&self, item: ResponseOutputItem) -> Vec<GeminiGenerateContentResponseBody> {
        let mut out = Vec::new();
        match item {
    ...
    pub fn on_stream_event(
        ...
        match event {
            ...
            | ResponseStreamEvent::OutputItemDone { item, .. } => {
                for chunk in self.map_output_item(item) {
                    out.push(chunk_event(chunk));
                }
            }
...
impl TryFrom<OpenAiCreateResponseSseStreamBody> for GeminiSseStreamBody {
    type Error = TransformError;

    fn try_from(value: OpenAiCreateResponseSseStreamBody) -> Result<Self, TransformError> {
        let mut converter = OpenAiResponseToGeminiStream::default();
...
impl TryFrom<OpenAiCreateResponseSseStreamBody> for GeminiNdjsonStreamBody {
    type Error = TransformError;
