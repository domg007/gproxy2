...
#[cfg(test)]
mod tests {
    use super::gemini_parameters_schema_to_claude_input_schema;
    use serde_json::json;

    #[test]
    fn parses_json_schema_without_leaking_meta_keys_into_properties() {
        let schema = json!({
        ...
        let converted = gemini_parameters_schema_to_claude_input_schema(schema);
        let properties = converted.properties.expect("properties should exist");

        assert!(properties.contains_key("query"));
        assert!(properties.contains_key("limit"));
        assert!(!properties.contains_key("type"));
        assert!(!properties.contains_key("required"));
        assert_eq!(converted.required, Some(vec!["query".to_string()]));
        assert_eq!(
            converted.extra_fields.get("additionalProperties"),
