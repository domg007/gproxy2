...
mod tests {
    use super::*;
    use crate::claude::count_tokens::types as ct;
    use crate::claude::create_message::request::RequestBody as ClaudeRequestBody;
    use crate::claude::create_message::types::Model as ClaudeModel;

    fn base_request(
        messages: Vec<ct::BetaMessageParam>,
        tools: Option<Vec<ct::BetaToolUnion>>,
        tool_choice: Option<ct::BetaToolChoice>,
    ) -> ClaudeCreateMessageRequest {
        ClaudeCreateMessageRequest {
            body: ClaudeRequestBody {
                max_tokens: 512,
                messages,
                model: ClaudeModel::Custom("claude-sonnet-4-6".to_string()),
                tools,
                tool_choice,
                ..ClaudeRequestBody::default()
            },
            ..ClaudeCreateMessageRequest::default()
        }
    }

    fn custom_tool(name: &str) -> ct::BetaToolUnion {
    ...
    fn bash_tool() -> ct::BetaToolUnion {
        ct::BetaToolUnion::Bash20250124(ct::BetaToolBash20250124 {
    ...
    fn apply_patch_tool() -> ct::BetaToolUnion {
        ct::BetaToolUnion::TextEditor20250728(ct::BetaToolTextEditor20250728 {
    ...
    fn file_search_tool() -> ct::BetaToolUnion {
        ct::BetaToolUnion::ToolSearchBm25_20251119(ct::BetaToolSearchToolBm25_20251119 {
    ...
    fn mcp_toolset(name: &str) -> ct::BetaToolUnion {
        ct::BetaToolUnion::McpToolset(ct::BetaMcpToolset {
    ...
    fn tool_choice(name: &str) -> ct::BetaToolChoice {
        ct::BetaToolChoice::Tool(ct::BetaToolChoiceTool {
            name: name.to_string(),
    ...
    #[test]
    fn claude_tool_choice_maps_to_native_openai_variants() {
        let shell = OpenAiCreateResponseRequest::try_from(base_request(
            Vec::new(),
            Some(vec![bash_tool()]),
            Some(tool_choice("bash")),
        ))
        .expect("bash request should convert");
        assert!(matches!(
            shell.body.tool_choice,
            Some(ResponseToolChoice::Shell(_))
        ));

        let apply_patch = OpenAiCreateResponseRequest::try_from(base_request(
            Vec::new(),
            Some(vec![apply_patch_tool()]),
            Some(tool_choice("str_replace_based_edit_tool")),
        ))
        .expect("apply_patch request should convert");
        assert!(matches!(
            apply_patch.body.tool_choice,
            Some(ResponseToolChoice::ApplyPatch(_))
        ));

        let file_search = OpenAiCreateResponseRequest::try_from(base_request(
            Vec::new(),
            Some(vec![file_search_tool()]),
            Some(tool_choice("tool_search_tool_bm25")),
        ))
        .expect("file_search request should convert");
        assert!(matches!(
            file_search.body.tool_choice,
            Some(ResponseToolChoice::Types(choice))
                if choice.type_ == crate::openai::count_tokens::types::ResponseToolChoiceBuiltinType::FileSearch
        ));

        let custom = OpenAiCreateResponseRequest::try_from(base_request(
            Vec::new(),
            Some(vec![custom_tool("apply_patch")]),
            Some(tool_choice("apply_patch")),
        ))
        .expect("custom request should convert");
        match custom.body.tool_choice {
            Some(ResponseToolChoice::Custom(choice)) => assert_eq!(choice.name, "apply_patch"),
            other => panic!("unexpected custom tool choice: {other:?}"),
        }

        let mcp = OpenAiCreateResponseRequest::try_from(base_request(
            Vec::new(),
            Some(vec![mcp_toolset("repo")]),
            Some(tool_choice("repo")),
        ))
        .expect("mcp request should convert");
        match mcp.body.tool_choice {
            Some(ResponseToolChoice::Mcp(choice)) => {
                assert_eq!(choice.server_label, "repo");
                assert_eq!(choice.name, None);
            }
            other => panic!("unexpected mcp tool choice: {other:?}"),
        }
    }

    #[test]
    fn claude_native_tool_results_fill_openai_input_items() {
        let request = base_request(
            vec![
                ct::BetaMessageParam {
                    ...
                    content: ct::BetaMessageContent::Blocks(vec![
                        ...
                        ct::BetaContentBlockParam::ToolUse(ct::BetaToolUseBlockParam {
                            ...
                            .expect("custom input should parse"),
                ...
                ct::BetaMessageParam {
                    ...
                    content: ct::BetaMessageContent::Blocks(vec![
                        ct::BetaContentBlockParam::WebSearchToolResult(
                            ct::BetaWebSearchToolResultBlockParam {
                                content: ct::BetaWebSearchToolResultBlockParamContent::Results(
                                    vec![ct::BetaWebSearchResultBlockParam {
                                        encrypted_content: "enc".to_string(),
                                        title: "Rust".to_string(),
                                        type_: ct::BetaWebSearchResultBlockType::WebSearchResult,
                                        url: "https://www.rust-lang.org".to_string(),
                                        page_age: Some("1d".to_string()),
                                    }],
                                ),
                                tool_use_id: "web_1".to_string(),
                                type_: ct::BetaWebSearchToolResultBlockType::WebSearchToolResult,
                                cache_control: None,
                            },
                        ),
                        ct::BetaContentBlockParam::ToolResult(ct::BetaToolResultBlockParam {
                            tool_use_id: "custom_1".to_string(),
            ...
            Some(vec![
                ...
                mcp_toolset("repo"),
                ct::BetaToolUnion::WebSearch20250305(ct::BetaWebSearchTool20250305 {
        ...
        let converted = OpenAiCreateResponseRequest::try_from(request)
            .expect("claude request should convert to openai response request");
        ...
        match &items[0] {
            ResponseInputItem::FunctionWebSearch(call) => match &call.action {
                crate::openai::count_tokens::types::ResponseFunctionWebSearchAction::Search {
                    ...
                    assert_eq!(call.status, crate::openai::count_tokens::types::ResponseFunctionWebSearchStatus::Completed);
        ...
        match &items[1] {
            ResponseInputItem::CustomToolCall(call) => assert_eq!(call.name, "apply_patch"),
            other => panic!("unexpected custom tool call: {other:?}"),
        }

        match &items[2] {
            ResponseInputItem::McpCall(call) => {
                assert_eq!(call.server_label, "repo");
                assert_eq!(call.output.as_deref(), Some("done"));
                assert_eq!(
                    call.status,
                    Some(crate::openai::count_tokens::types::ResponseToolCallStatus::Completed)
                );
            }
            other => panic!("unexpected mcp call: {other:?}"),
        }

        match &items[3] {
            ResponseInputItem::CustomToolCallOutput(call) => match &call.output {
                crate::openai::count_tokens::types::ResponseCustomToolCallOutputContent::Text(
                    text,
                ) => {
            ...
            other => panic!("unexpected custom tool call output: {other:?}"),
        }
    }
}
