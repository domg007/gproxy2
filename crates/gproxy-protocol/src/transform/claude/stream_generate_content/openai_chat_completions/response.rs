...
use crate::openai::create_chat_completions::stream::{
    ChatCompletionChunk, OpenAiChatCompletionsSseData, OpenAiChatCompletionsSseEvent,
...
impl OpenAiChatCompletionsToClaudeStream {
    ...
    pub fn on_event(
        ...
        Ok(match event.data {
            OpenAiChatCompletionsSseData::Done(_) => self.finish(),
            OpenAiChatCompletionsSseData::Chunk(chunk) => self.on_chunk(chunk),
        })
    }

    pub fn on_chunk(&mut self, chunk: ChatCompletionChunk) -> Vec<ClaudeCreateMessageStreamEvent> {
        if self.is_finished() {
            return Vec::new();
        }
...
impl TryFrom<OpenAiChatCompletionsSseStreamBody> for Vec<ClaudeCreateMessageStreamEvent> {
...
impl TryFrom<OpenAiChatCompletionsSseStreamBody> for ClaudeCreateMessageSseStreamBody {
    type Error = TransformError;
