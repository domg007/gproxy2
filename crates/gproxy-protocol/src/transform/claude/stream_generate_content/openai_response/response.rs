...
impl OpenAiResponseToClaudeStream {
    ...
    fn close_tool_block(&mut self, out: &mut Vec<ClaudeCreateMessageStreamEvent>, item_id: &str) {
    ...
    pub fn on_event(
        ...
        Ok(match event.data {
            OpenAiCreateResponseSseData::Done(_) => self.finish(),
            OpenAiCreateResponseSseData::Event(stream_event) => self.on_stream_event(stream_event),
        })
    }
...
impl TryFrom<OpenAiCreateResponseSseStreamBody> for Vec<ClaudeCreateMessageStreamEvent> {
...
impl TryFrom<OpenAiCreateResponseSseStreamBody> for ClaudeCreateMessageSseStreamBody {
    type Error = TransformError;
