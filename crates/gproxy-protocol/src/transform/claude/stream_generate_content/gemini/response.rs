...
impl GeminiToClaudeStream {
    ...
    fn update_envelope_from_chunk(&mut self, chunk: &GeminiGenerateContentResponseBody) {
        ...
        if let Some(usage_metadata) = chunk.usage_metadata.as_ref() {
            self.input_tokens = usage_metadata
                .prompt_token_count
                .unwrap_or(0)
                .saturating_add(usage_metadata.tool_use_prompt_token_count.unwrap_or(0));
            self.cached_input_tokens = usage_metadata.cached_content_token_count.unwrap_or(0);
            self.output_tokens = usage_metadata
    ...
    fn ensure_running(&mut self, out: &mut Vec<ClaudeCreateMessageStreamEvent>) {
    ...
    fn emit_text_block(&mut self, out: &mut Vec<ClaudeCreateMessageStreamEvent>, text: String) {
    ...
    pub fn on_sse_event(
    ...
    ) -> Result<Vec<ClaudeCreateMessageStreamEvent>, TransformError> {
        if self.is_finished() {
            return Ok(Vec::new());
        }
        ...
        Ok(match event.data {
            ...
            GeminiSseEventData::Done(_) => self.finish(),
        })
    }
    ...
    ) -> Vec<ClaudeCreateMessageStreamEvent> {
        if self.is_finished() {
            return Vec::new();
        }
    ...
    pub fn finish(&mut self) -> Vec<ClaudeCreateMessageStreamEvent> {
        if self.is_finished() {
            return Vec::new();
        }
...
impl TryFrom<GeminiStreamGenerateContentResponse> for Vec<ClaudeCreateMessageStreamEvent> {
    type Error = TransformError;

    fn try_from(value: GeminiStreamGenerateContentResponse) -> Result<Self, TransformError> {
        let mut converter = GeminiToClaudeStream::default();
        let mut out = Vec::new();

        match value {
            GeminiStreamGenerateContentResponse::NdjsonSuccess { body, .. } => {
                for chunk in body.chunks {
                    out.extend(converter.on_chunk(chunk));
                }
            }
            GeminiStreamGenerateContentResponse::SseSuccess { body, .. } => {
                for event in body.events {
                    out.extend(converter.on_sse_event(event)?);
                }
            }
            GeminiStreamGenerateContentResponse::Error { body, .. } => {
                return Ok(vec![stream_error_event(body.error.message)]);
            }
        }
...
impl TryFrom<GeminiStreamGenerateContentResponse> for ClaudeCreateMessageSseStreamBody {
    type Error = TransformError;
...
impl TryFrom<GeminiSseStreamBody> for Vec<ClaudeCreateMessageStreamEvent> {
    type Error = TransformError;
...
impl TryFrom<GeminiNdjsonStreamBody> for Vec<ClaudeCreateMessageStreamEvent> {
    type Error = TransformError;
...
impl TryFrom<GeminiSseStreamBody> for ClaudeCreateMessageSseStreamBody {
    type Error = TransformError;
...
impl TryFrom<GeminiNdjsonStreamBody> for ClaudeCreateMessageSseStreamBody {
    type Error = TransformError;
