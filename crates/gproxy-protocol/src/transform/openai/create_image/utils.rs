...
pub(crate) fn image_stream_context_from_response_stream(
    stream: &crate::openai::create_response::stream::OpenAiCreateResponseSseStreamBody,
    preferred_action: PreferredImageAction,
) -> ImageStreamContext {
    ...
    for event in &stream.events {
        match &event.data {
            crate::openai::create_response::stream::OpenAiCreateResponseSseData::Done(_) => {}
            crate::openai::create_response::stream::OpenAiCreateResponseSseData::Event(event) => {
                match event {
