...
use crate::openai::create_response::stream::{
    OpenAiCreateResponseSseEvent, OpenAiCreateResponseSseStreamBody, ResponseStreamContentPart,
    ResponseStreamEvent,
};
...
impl GeminiToOpenAiResponseStream {
    ...
    fn ensure_started(&mut self, out: &mut Vec<OpenAiCreateResponseSseEvent>) {
        if self.started {
            return;
        }
    ...
    fn close_message(&mut self, out: &mut Vec<OpenAiCreateResponseSseEvent>, output_index: u64) {
    ...
    fn close_reasoning(&mut self, out: &mut Vec<OpenAiCreateResponseSseEvent>, output_index: u64) {
    ...
    fn on_chunk(
    ...
    ) -> Result<Vec<OpenAiCreateResponseSseEvent>, TransformError> {
        if self.finished {
            return Ok(Vec::new());
        }
        ...
        {
            self.emit_message_delta(&mut out, 0, refusal_text, true);
        }
        ...
        if let Some(candidates) = chunk.candidates {
            for (candidate_pos, candidate) in candidates.into_iter().enumerate() {
                    ...
                    for (part_index, part) in content.parts.into_iter().enumerate() {
                        if part.thought.unwrap_or(false) {
                            if let Some(text) = part.text {
                                ...
                                self.emit_reasoning_delta(&mut out, output_index, item_id, text);
                            }
                            continue;
                        }
                        ...
                        if let Some(function_response) = part.function_response {
                            ...
                            self.emit_message_delta(&mut out, output_index, output_text, false);
                            continue;
                        }
                        ...
                        if let Some(text) = part.text {
                            self.emit_message_delta(&mut out, output_index, text, false);
                            continue;
                        }
                ...
                {
                    self.emit_message_delta(&mut out, output_index, finish_message, false);
                }
                ...
                if let Some(finish_reason) = candidate.finish_reason.as_ref() {
                    ...
                    self.finish_output_index(&mut out, output_index);
                }
            }
        }
    ...
    fn close_all_open_items(&mut self) -> Vec<OpenAiCreateResponseSseEvent> {
        let mut out = Vec::new();
        ...
        for call_id in remaining_call_ids {
            self.close_function_call(&mut out, call_id);
        }
    ...
    fn finalize(
        ...
        error: Option<rt::ResponseError>,
    ) -> Vec<OpenAiCreateResponseSseEvent> {
        if self.finished {
            return Vec::new();
        }
        ...
        let mut out = Vec::new();
        self.ensure_started(&mut out);
        out.extend(self.close_all_open_items());
...
impl TryFrom<GeminiStreamGenerateContentResponse> for OpenAiCreateResponseSseStreamBody {
    type Error = TransformError;

    fn try_from(value: GeminiStreamGenerateContentResponse) -> Result<Self, TransformError> {
        let mut converter = GeminiToOpenAiResponseStream::default();
        ...
        match value {
            ...
            GeminiStreamGenerateContentResponse::Error { body, .. } => {
                let mut out = Vec::new();
                converter.ensure_started(&mut out);
                ...
                let error_sequence = next_sequence_number(&mut converter.next_sequence_number);
                push_stream_event(
                    &mut out,
                ...
                out.extend(converter.finalize(
                    rt::ResponseStatus::Failed,
                    Some(rt::ResponseError {
                        code: response_error_code_from_gemini_status(&code),
                        message,
                    }),
                ));
                events.extend(out);
...
impl TryFrom<GeminiSseStreamBody> for OpenAiCreateResponseSseStreamBody {
    type Error = TransformError;
...
impl TryFrom<GeminiNdjsonStreamBody> for OpenAiCreateResponseSseStreamBody {
    type Error = TransformError;
