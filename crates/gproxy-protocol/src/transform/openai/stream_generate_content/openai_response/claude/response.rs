...
impl ClaudeToOpenAiResponseStream {
    ...
    fn on_content_block_start(
        ...
        match content_block {
            BetaContentBlock::Text(block) => {
                ...
                let text = block.text;
                self.emit_message_part_added(&mut out, item_id.clone(), index, text.clone());
        ...
        match (self.blocks.get_mut(&index), delta) {
            (
                ...
                if !delta.text.is_empty() {
                    ...
                    text_delta = Some((item_id.clone(), delta.text));
                }
            }
            (
            ...
            (
                ...
                *encrypted_content = delta.content.unwrap_or_default();
            }
            _ => {}
        }
    ...
    fn on_content_block_stop(&mut self, index: u64) -> Vec<OpenAiCreateResponseSseEvent> {
        if self.finished {
            return Vec::new();
        }
    ...
    fn close_all_open_blocks(&mut self) -> Vec<OpenAiCreateResponseSseEvent> {
        let mut out = Vec::new();
        let indexes = self.blocks.keys().copied().collect::<Vec<_>>();
        for index in indexes {
            out.extend(self.on_content_block_stop(index));
        }
        out
    }
    ...
    fn finalize(
        ...
        let mut out = Vec::new();
        self.ensure_started(&mut out);
        out.extend(self.close_all_open_blocks());
        ...
        Ok(match event {
            ClaudeCreateMessageStreamEvent::MessageStart(event) => {
                ...
                self.incomplete_reason =
                    stop_reason_to_incomplete_reason(event.message.stop_reason);
            ...
            ClaudeCreateMessageStreamEvent::ContentBlockStart(event) => {
                self.on_content_block_start(event.index, event.content_block)
            }
            ClaudeCreateMessageStreamEvent::ContentBlockDelta(event) => {
                self.on_content_block_delta(event.index, event.delta)
            }
            ClaudeCreateMessageStreamEvent::ContentBlockStop(event) => {
                self.on_content_block_stop(event.index)
            }
            ClaudeCreateMessageStreamEvent::MessageDelta(event) => {
                ...
                if event.delta.stop_reason.is_some() {
                    self.incomplete_reason =
                        stop_reason_to_incomplete_reason(event.delta.stop_reason);
                }
            ...
            ClaudeCreateMessageStreamEvent::MessageStop(_) => self.finish(),
            ClaudeCreateMessageStreamEvent::Error(event) => {
                let (code, message) = beta_error_message_and_code(event.error);
                let mut out = Vec::new();
                self.ensure_started(&mut out);
                ...
                out.extend(self.finalize(rt::ResponseStatus::Failed, Some(response_error)));
                out
            }
            ClaudeCreateMessageStreamEvent::Ping(_)
            | ClaudeCreateMessageStreamEvent::Unknown(_) => Vec::new(),
        })
    }
}

impl TryFrom<Vec<ClaudeCreateMessageStreamEvent>> for OpenAiCreateResponseSseStreamBody {
...
impl TryFrom<ClaudeCreateMessageSseStreamBody> for OpenAiCreateResponseSseStreamBody {
    type Error = TransformError;
