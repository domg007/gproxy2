...
use crate::openai::create_chat_completions::stream::{
    ...
    OpenAiChatCompletionsSseData, OpenAiChatCompletionsSseEvent,
...
impl OpenAiChatCompletionsToOpenAiResponseStream {
    ...
    fn close_message(&mut self, out: &mut Vec<OpenAiCreateResponseSseEvent>, choice_index: u32) {
    ...
    pub fn on_event(
        ...
        match event.data {
            ...
            OpenAiChatCompletionsSseData::Done(_) => Ok(self.finish()),
        }
    }
    ...
    fn close_all_open_items(&mut self) -> Vec<OpenAiCreateResponseSseEvent> {
        ...
        for choice_index in choice_indexes {
            self.finish_choice(&mut out, choice_index);
        }

        out
    }

    fn finalize(&mut self) -> Vec<OpenAiCreateResponseSseEvent> {
        if self.finished {
            return Vec::new();
        }
