...
use crate::openai::create_chat_completions::stream::{
    ChatCompletionChunk, OpenAiChatCompletionsSseData, OpenAiChatCompletionsSseStreamBody,
};
...
impl TryFrom<OpenAiChatCompletionsSseStreamBody> for OpenAiChatCompletionsResponse {
    ...
    fn try_from(value: OpenAiChatCompletionsSseStreamBody) -> Result<Self, TransformError> {
        ...
        for event in value.events {
            match event.data {
                ...
                OpenAiChatCompletionsSseData::Done(_) => break,
            }
        }
...
mod tests {
    ...
    #[test]
    fn stream_to_nonstream_preserves_reasoning_details() {
        let stream = OpenAiChatCompletionsSseStreamBody {
            events: vec![
                OpenAiChatCompletionsSseEvent {
                    event: None,
