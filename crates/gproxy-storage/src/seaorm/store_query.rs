...
use crate::query::{
    ...
    ProviderQueryRow, Scope, UpstreamRequestQuery, UpstreamRequestQueryRow, UsageQuery,
    UsageQueryRow, UsageSummary, UserKeyMemoryRow, UserKeyQuery, UserKeyQueryRow, UserQuery,
    UserQueryRow,
};
...
impl SeaOrmStorage {
    ...
    pub async fn query_upstream_requests(
        ...
        let mut stmt =
            upstream_requests::Entity::find().order_by(upstream_requests::Column::At, Order::Desc);
        if let Scope::Eq(provider_id) = query.provider_id {
            stmt = stmt.filter(upstream_requests::Column::ProviderId.eq(provider_id));
        }
        if let Scope::Eq(credential_id) = query.credential_id {
            stmt = stmt.filter(upstream_requests::Column::CredentialId.eq(credential_id));
        }
        if let Some(from_unix_ms) = query.from_unix_ms
            && let Ok(from) = unix_ms_to_offset_datetime(from_unix_ms)
        {
            stmt = stmt.filter(upstream_requests::Column::At.gte(from));
        }
        if let Some(to_unix_ms) = query.to_unix_ms
            && let Ok(to) = unix_ms_to_offset_datetime(to_unix_ms)
        {
            stmt = stmt.filter(upstream_requests::Column::At.lt(to));
        }
        if let Some(limit) = query.limit
            && limit > 0
        {
    ...
    pub async fn query_downstream_requests(
        ...
        if let Scope::Eq(user_id) = query.user_id {
            stmt = stmt.filter(downstream_requests::Column::UserId.eq(user_id));
        }
        if let Scope::Eq(user_key_id) = query.user_key_id {
            stmt = stmt.filter(downstream_requests::Column::UserKeyId.eq(user_key_id));
        }
        if let Some(from_unix_ms) = query.from_unix_ms
            && let Ok(from) = unix_ms_to_offset_datetime(from_unix_ms)
        {
            stmt = stmt.filter(downstream_requests::Column::At.gte(from));
        }
        if let Some(to_unix_ms) = query.to_unix_ms
            && let Ok(to) = unix_ms_to_offset_datetime(to_unix_ms)
        {
            stmt = stmt.filter(downstream_requests::Column::At.lt(to));
        }
        if let Some(limit) = query.limit
            && limit > 0
        {
